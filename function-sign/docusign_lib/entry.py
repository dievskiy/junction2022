...
from docusign_lib.app.jwt_helpers import get_jwt_token, get_private_key
from docusign_lib.app.eSignature.examples.eg002_signing_via_email import Eg002SigningViaEmailController
from docusign_lib.app.jwt_config import DS_JWT
...
def create_args(api_account_id, access_token, base_path, data):
    envelope_args = {
...
def run_example(private_key, api_client, data):
    jwt_values = get_token(private_key, api_client)
    args = create_args(jwt_values["api_account_id"], jwt_values["access_token"], jwt_values["base_path"], data)
...
def send_email(data):
    ...
    private_key = DS_JWT["private_key"].encode("ascii").decode("utf-8")

    try:
        run_example(private_key, api_client, data)
